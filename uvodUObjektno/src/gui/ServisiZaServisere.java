...
public class ServisiZaServisere extends JFrame {
	...
	public ServisiZaServisere(Serviser serviser, Servisi servisi, Korisnici korisnici, Automobili automobili) {
	...
	private void initGUI() {
		...
		ArrayList<Servis> servisiNovi = null;
		if (serviser instanceof Serviser) {
			servisiNovi = new ArrayList<Servis>();
			for(Servis servis : servisi.sviNeobrisaniServisi()) {
				if(servis.getServiser().getKorisnickoIme().equals(serviser.getKorisnickoIme())) {
					servisiNovi.add(servis);
				}
			}
		}
		...
		Object[][] sadrzaj = new Object[servisiNovi.size()][zaglavlja.length];
		
		for(int i=0; i<servisiNovi.size(); i++) {
			Servis servis = servisiNovi.get(i);
